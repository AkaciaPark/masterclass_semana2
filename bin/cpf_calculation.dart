class CpfNumbers {
  ...
  CpfNumbers({required this.cpf});

  String? cpfValidation() {
    final regexValidation = _regexValidation();
    ...
    if (firstValidation == false)
      return 'CPF inválido no primeiro dígito verificador';

    final secondValidation =
        _validateVerifierDigit(cpf: handleCpf, indexToValidate: 10);
    if (secondValidation == false)
      return 'CPF inválido no segundo dígito verificador';

    return null;
  }

  String? _regexValidation() {
    if (cpf.trim().isEmpty) {
